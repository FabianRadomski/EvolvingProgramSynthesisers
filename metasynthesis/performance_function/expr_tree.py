...
class ExpressionTree:
    ...
    def random_walk_helper(self, cur_depth, min_depth, traversal) -> Tuple[List[int], 'ExpressionTree']:
        # returns a random subtree
        stop: bool = get_next_bernoulli(p=0.5)  # true with probability p
        if (stop and (cur_depth >= min_depth)) or (not self.symbol.is_operator()):
            ...
            return pick_child.random_walk_helper(cur_depth=cur_depth + 1, min_depth=min_depth, traversal=traversal)

    def random_walk(self, min_depth=0) -> Tuple[List[int], 'ExpressionTree']:
        return self.random_walk_helper(cur_depth=0, min_depth=min_depth, traversal=[])
    ...
    def mutate_tree(self, verbose=False) -> 'ExpressionTree':
        ...
        traversal, random_node = clone.random_walk(min_depth=0)
    ...
    def reproduce_with(self, other: 'ExpressionTree', verbose=False) -> Tuple['ExpressionTree', 'ExpressionTree']:
        traversal_t, t = self.random_walk(min_depth=1)
        if verbose:
...
def helper_generator(terms, operators, cur_depth, max_depth) -> ExpressionTree:
    if cur_depth == max_depth:
        ...
        # min depth = 1
        probability_of_operator = 1 if (cur_depth == 0) else 0.5
