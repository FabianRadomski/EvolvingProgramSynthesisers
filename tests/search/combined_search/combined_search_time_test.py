...
class CombinedSearchTests(unittest.TestCase):
    ...
    def run_combined(self, alg_seq, setting):
        ...
        avg_success, average_time, _ = runner.run()
    ...
    def test_longer_chain(self):
        success_combined = self.run_combined([("Brute", 1), ("AS", 1), ("MH", 1), ("LNS", 1), ("MCTS", 1)], self.setting)
        success1 = self.run_solo("Brute", 1, self.setting)
        success2 = self.run_solo("AS", 1, self.setting)
        success3 = self.run_solo("MH", 1, self.setting)
        success4 = self.run_solo("LNS", 1, self.setting)
        success5 = self.run_solo("MCTS", 1, self.setting)
        self.assertTrue(success_combined >= success1)
        self.assertTrue(success_combined >= success2)
