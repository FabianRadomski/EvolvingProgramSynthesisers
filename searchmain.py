...
def plot_fitness(evolutions_history):
    ...
    plt.title(f"Fitness over Generations in the Pixel Domain")
    ax.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
    ax.xaxis.set_major_locator(MaxNLocator(integer=True))
    plt.legend()
    plt.savefig(f"fit-pixels.png")
    plt.show()
...
def plot_successes(evolutions_history, mutations_results):
    ...
    plt.title(f"Average Success Rate over Generations in the Pixel Domain\n")
    ax.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
    ax.xaxis.set_major_locator(MaxNLocator(integer=True))
    plt.legend()
    plt.savefig(f"succ-pixels.png")
    plt.show()
...
def plot_speeds(speeds):
    ...
    plt.xlabel("Generation")
    plt.ylabel("Time")
    plt.title(f"Average Execution Time of Search Procedures in the Pixel Domain")
    ax.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
    ax.xaxis.set_major_locator(MaxNLocator(integer=True))
    plt.legend()
    plt.savefig(f"exec-pixels.png")
    plt.show()
...
if __name__ == "__main__":
    ...
    for mut in mutation_probabilities:
        ss = SearchSynthesiser(fitness_limit=0, generation_limit=30, crossover_probability=0.8,
                               mutation_probability=mut, generation_size=20, max_seq_size=6, dist_type="Time", print_generations=True,
                               setting="PO", test_size="param", plot=True, write_generations=True)
